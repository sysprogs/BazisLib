...
namespace BazisLib
{
	namespace Win32
	{
		...
		public:
			...
			{
				return value.__SERIALIZER_ITERATE_MEMBERS(Worker(key), true);
			}
		...
		private:
			...
			public:
				...
				template <typename _Ty> ActionStatus inline _NestedSerializerHelper(TCHAR *pName, _Ty *pObject, bool save, void *)
				{
					if (save)
				...
				template <typename _Ty> ActionStatus inline _NestedSerializerHelper(TCHAR *pName, _Ty *pObject, bool save, _SerializableStructBase *)
				{
					RegistryKey subKey(Key[pName], save);
					return pObject->__SERIALIZER_ITERATE_MEMBERS(Worker(subKey), save);
				}

				template <typename _Ty> ActionStatus inline _SerializerEntry(TCHAR *pName, _Ty *pObject, bool save)
				{
